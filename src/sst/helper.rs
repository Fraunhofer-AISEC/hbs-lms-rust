...
pub fn get_sst_first_leaf_node_idx(signing_entity_idx: u8, total_height: u8, top_div: u8) -> u32 {
    assert!(signing_entity_idx > 0);
    assert!(signing_entity_idx as u32 <= _get_num_leaves(top_div)); // number of nodes at that level
...
pub fn get_sst_last_leaf_node_idx(signing_entity_idx: u8, total_height: u8, top_div: u8) -> u32 {
    assert!(signing_entity_idx > 0);
    assert!(signing_entity_idx as u32 <= _get_num_leaves(top_div)); // number of nodes at that level
    get_sst_first_leaf_node_idx(signing_entity_idx, total_height, top_div) - 1 + get_num_leaves_in_sst(total_height, top_div)
}
...
mod tests {
    ...
    */
    ...
    fn test_get_sst_first_leaf_idx_1() {
        let first_leaf_idx = get_sst_first_leaf_node_idx(1, 5, 3);
        assert_eq!(32, first_leaf_idx);
    }

    #[test]
    fn test_get_sst_first_leaf_idx_2() {
        let first_leaf_idx = get_sst_first_leaf_node_idx(3, 5, 3);
        assert_eq!(40, first_leaf_idx);
    }

    #[test]
    fn test_get_sst_first_leaf_idx_3() {
        let first_leaf_idx = get_sst_first_leaf_node_idx(8, 5, 3);
        assert_eq!(60, first_leaf_idx);
    }

    #[test]
    fn test_get_sst_last_leaf_idx_1() {
        let last_leaf_idx = get_sst_last_leaf_node_idx(1, 5, 3);
        assert_eq!(35, last_leaf_idx);
    }

    #[test]
    fn test_get_sst_last_leaf_idx_2() {
        let last_leaf_idx = get_sst_last_leaf_node_idx(3, 5, 3);
        assert_eq!(43, last_leaf_idx);
    }

    #[test]
    fn test_get_sst_last_leaf_idx_3() {
        let last_leaf_idx = get_sst_last_leaf_node_idx(8, 5, 3);
    ...
    fn test_get_sst_last_leaf_idx_4() {
        let last_leaf_idx = get_sst_last_leaf_node_idx(9, 5, 3);
        assert_eq!(63, last_leaf_idx);
    }
